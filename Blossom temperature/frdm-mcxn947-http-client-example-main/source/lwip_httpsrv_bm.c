...
int main(void)
{
    ...
    /* Make an HTTP POST request. */
    {
        ...
        IP_ADDR4(&demo_server_ip, 10, 14, 120, 87);
        const char *demo_body = "{\"hello\":\"world\"}";
        err_t err = http_client_post(&demo_server_ip, 8080,
                                     "/api/data",
                                     "application/json",
                                     demo_body, (u16_t)strlen(demo_body),
        ...
        {
            PRINTF("HTTP Client POST request initiated to %s:8080\r\n", ipaddr_ntoa(&demo_server_ip));
        }
    }
